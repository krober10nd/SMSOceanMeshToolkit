...
class Grid(Region):
    ...
    def __init__(
        ...
        self.nx = int(np.ceil((self.bbox[1] - self.bbox[0]) / self.dx)) + 1
        self.ny = int(np.ceil((self.bbox[3] - self.bbox[2]) / self.dy)) + 1
        ...
        self.dx = _xvec[1] - _xvec[0]
        self.dy = _yvec[1] - _yvec[0]
        self.nx = len(_xvec)
        self.ny = len(_yvec)
